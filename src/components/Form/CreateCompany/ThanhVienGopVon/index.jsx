	...
	useEffect(() => {
		if (present === 'organization') {
			ref.current.setFields([
				{
					name: ['create_company', 'approve', 'origin_person', 'company', 'national'],
					value: 'Việt Nam',
				},
			]);
		}
		...
		if (present === 'personal') {
			xhtml = (
				<div className={styles.groupInput}>
					<CCInput name={[...BASE_FORM, 'origin_person', 'name']} label='Họ và Tên' placeholder='NGUYỄN VĂN A' />
					...
					<CCSelect.SelectPersonType ref={ref} name={[...BASE_FORM, 'origin_person', 'per_type']} label='Dân tộc' />
					...
					<Form.Item label='Địa chỉ liên lạc' className={styles.newLine}>
						...
						{radio === 2 && (
							<div style={{ padding: '8px 0' }}>
								<CCSelect.SelectProvince ref={ref} name={[...BASE_FORM, 'origin_person', 'contact']} label='Nơi cấp' />
							</div>
						)}
					</Form.Item>
				</div>
			...
			xhtml = (
				<div className={styles.groupInput}>
					...
					<CCInput
						...
						label={<div dangerouslySetInnerHTML={{ __html: '</>Ngày cấp <i>(ngày đăng ký lần đầu)</i></>' }} />}
						placeholder='Chọn ngày'
					/>
					<CCInput
					...
					<Form.Item label='Địa chỉ trụ sở chính' className={styles.newLine}>
						<CCSelect.SelectProvince ref={ref} name={[...BASE_FORM, 'organization_name', 'company']} label='Nơi cấp' />
					...
					<CCInput
						...
						label={
							<div
								dangerouslySetInnerHTML={{ __html: '</>Họ và Tên người đại diện theo pháp luật <i>(ĐDPL)</i></>' }}
							/>
						}
					...
					<CCInput
						...
						label={<div dangerouslySetInnerHTML={{ __html: '</>Chức danh <i>(ĐDPL)</i></>' }} />}
					...
					<Form.Item
						...
						// name={[...BASE_FORM, 'origin_person', 'title']}
						label={<div dangerouslySetInnerHTML={{ __html: '</>Địa chỉ thường trú <i>(ĐDPL)</i></>' }} />}
					>
						{/* <CCInput
						...
						<CCSelect.SelectProvince ref={ref} name={[...BASE_FORM, 'origin_person', 'company']} label='Nơi cấp' />

						{/* <CCInput name={[...BASE_FORM, "origin_person", "company", "national"]} label="Quốc gia" /> */}
					</Form.Item>

					<Form.Item
						label={<div dangerouslySetInnerHTML={{ __html: '</>Địa chỉ liên lạc <i>(ĐDPL)</i></>' }} />}
						className={styles.newLine}
					>
						{/* <CCInput
